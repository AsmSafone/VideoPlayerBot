...
\u2022 `/setvar` - set/change heroku config vars
...
async def set_heroku_var(client, message):
    ...
    if " " in message.text:
        ...
        await asyncio.sleep(2)
        await m.edit(f"✅ **Succesfully Set {var} With Value {value}, Now Restarting To Apply Changes !**")
        config[var] = str(value)
    else:
        await message.reply_text("❗ **You Haven't Provided Any Value For Variable, You Should Follow The Correct Format !** \n\nFor Example: \n• `/setvar CHAT_ID=-1001313215676` to change or set CHAT_ID var. \n• `/setvar REPLY_MESSAGE=` to delete REPLY_MESSAGE var.") 
